...
public class ftp { 
 ...
 DataSocket = new Socket(host, 2221); 
//  DataSocket = new Socket(host, 2121); 
 ipstream = new DataInputStream(DataSocket.getInputStream());  outstream = new PrintStream(DataSocket.getOutputStream());  String s = ipstream.readLine(); 
 if (s.startsWith("220")) { 
 ...
 System.out.println("Press Enter to continue . . . ");  String name = br.readLine(); 
 ...
 static void store() { 
 try { 
 BufferedReader br = new BufferedReader(new InputStreamReader(System.in));  System.out.println("Enter Filename: \n"); 
 ...
 StringTokenizer tokenizer = new StringTokenizer(dataLink, ",");  ip = tokenizer.nextToken() + "." + tokenizer.nextToken() + "." + tokenizer.nextToken() +  "." + tokenizer.nextToken(); 
 ...
 BufferedInputStream input = new BufferedInputStream(inputStream);  BufferedOutputStream output = new  
...
Integer.parseInt(tokenizer.nextToken()); 
 ...
 if (!s.startsWith("150")) { 
 throw new IOException("Simple FTP was not allowed to send the file:" + s);  } 
 BufferedInputStream input = new BufferedInputStream(dataSocket.getInputStream());  FileOutputStream output = new FileOutputStream("New" + filename);  byte[] buffer = new byte[4096]; 
 int bytesRead = 0; 
 while ((bytesRead = input.read(buffer)) != -1) { 
 output.write(buffer, 0, bytesRead); 
 } 
